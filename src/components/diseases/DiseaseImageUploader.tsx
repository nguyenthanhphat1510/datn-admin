...
import { deleteDiseaseImage, uploadDiseaseImages } from '@/lib/diseases-api';
...
interface Props {
  diseaseId?: string;
  existingImages: DiseaseImage[];
  pendingFiles: File[];
  onPendingFilesChange: (files: File[]) => void;
  onExistingChanged: () => void;
}

export default function DiseaseImageUploader({
  diseaseId,
  existingImages,
  pendingFiles,
  ...
  const handleFiles = async (fileList: FileList | null) => {
    ...
    if (diseaseId) {
      setWorking(true);
      try {
        await uploadDiseaseImages(diseaseId, files);
        onExistingChanged();
      } catch (err) {
        console.error(err);
  ...
  const removeExisting = async (publicId: string) => {
    if (!diseaseId) return;
    if (!confirm('Xóa ảnh này khỏi bệnh và khỏi Cloudinary?')) return;
    setWorking(true);
    setError(null);
    try {
      await deleteDiseaseImage(diseaseId, publicId);
      onExistingChanged();
    } catch (err) {
      console.error(err);
...
function Thumb({
  ...
  const labelClass = tone === 'pending' ? 'bg-amber-500/90' : 'bg-[#007e42]/85';
